...
public class PlatformMove : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        if (controller != null)
        {
            // controller 가 비어있지않다면, i.e. 플레이어의 캐릭터 컨트롤러의 정보가 담겼다면
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player")) // == if(other.gameObject.tag == "Player")
        {
            // Player가 맞다면 then 캐릭터 컨트롤러 정보를 담아줘
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.CompareTag("Player")) // == if(other.gameObject.tag == "Player")
        {
            // Player가 맞다면, Then 캐릭터 컨트롤러 정보를 지워줘
            controller = null;
        }
    }
}
